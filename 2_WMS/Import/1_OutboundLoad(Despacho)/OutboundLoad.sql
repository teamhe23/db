...
WHERE MODELO LIKE '%OS85100017781%'
    AND FEC_REG <= sysdate --TO_dATE('2025-01-01', 'YYYY-MM-DD')
    AND FEC_REG >= TO_dATE('2025-06-01', 'YYYY-MM-DD')
;

SELECT * FROM WMS_MODELO_REQUEST ORDER BY FEC_REG DESC FETCH FIRST 30 ROWS ONLY ;
SELECT * FROM WMS_MODELO_REQUEST WHERE IDENTIFICADOR IN ('OS85100015322');
SELECT * FROM WMS_MODELO_REQUEST WHERE IDENTIFICADOR IN ('OS85100015321');
SELECT * FROM WMS_MODELO_REQUEST WHERE IDENTIFICADOR IN ('OS85100015505');
SELECT * FROM WMS_SHP_LOAD_HDR ORDER BY CREATE_DATE DESC FETCH FIRST 20 ROWS ONLY ;
SELECT * FROM WMS_ERROR_INT; --ERRORES

--Validar Outbound Load
SELECT HDR.ERR_CODE, ERR.ERR_DESC, DOWNLOAD_DATE_1, HDR.* 
FROM WMS_SHP_LOAD_HDR HDR
         LEFT JOIN WMS_ERROR_INT ERR ON ERR.ERR_CODE = HDR.ERR_CODE
WHERE LOAD_MANIFEST_NBR IN ('OS85100015322')
    --ERR.ERR_CODE = 24;
;

SELECT * FROM wms_shp_load_hdr ORDER BY LOAD_MANIFEST_NBR DESC;
...
WHERE h.LOAD_MANIFEST_NBR = 'OS85100015322'
	AND d.ITEM_ALTERNATE_CODE = '10537'
;-- h.hdr_group_nbr = 609
...
/*
 * validacion e duplicado
 */
SELECT *
  from wms_shp_load_hdr h
 where h.load_manifest_nbr = 'OS85100015322'
   and h.hdr_group_nbr    != 1349 --number
